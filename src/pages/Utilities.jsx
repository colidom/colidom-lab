...
export default function Utilities() {
    ...
    const [activeToolId, setActiveToolId] = useState(toolId || utilityTools[0].id);
    const activeTool = utilityTools.find((tool) => tool.id === activeToolId);

    useEffect(() => {
        if (!activeTool) {
            navigate("/utilities/base64-encoder");
        }
    }, [activeTool, navigate]);
    ...
    return (
        <div id="utilities" className="flex flex-col md:flex-row min-h-screen">
            <Sidebar navItems={utilityTools} activeToolId={activeToolId} basePath="/utilities" />
